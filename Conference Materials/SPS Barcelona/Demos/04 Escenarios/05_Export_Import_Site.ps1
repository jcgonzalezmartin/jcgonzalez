...
function ExportImportSharePointSite
{
    ...
    try
    {
        ...
        "Export" {
            ...
            Write-Host "Export operation for $sSiteToExport successfully completed!!" -ForegroundColor Green

            } 
        "Import" {
            Write-Host "Doing the import for $sSiteToImport" -ForegroundColor Blue
            New-SPWeb -Url $sSiteToImport -Template "STS#0" -Name "Site Imported" -Language 1033 -AddToTopNav -UseParentTopNav #3082 (Spanish)
            Import-SPWeb -Identity $sSiteToImport -Path $sBackupPath
            Write-Host "Import Operation for $sSiteToImport successfully completed!!" -ForegroundColor Green
            }         
        default {
...
$sSiteToExport="http://c7370309033/sites/reports/"
$sSiteToImport="http://c7370309033/sites/reports/RBackup"
$sBackupPath="C:\Demos\SPS Lima\04 Escenarios\Reports.cmp"
#Export
